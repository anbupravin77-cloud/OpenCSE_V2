...
export function ThemeToggle({ className = '' }: { className?: string }) {
  ...
  return (
    ...
    >
      ...
      {theme === 'dark' ? (
        <Sun size={18} className="transition-transform duration-300 rotate-0 hover:rotate-45 text-amber-300 transform-gpu" />
      ) : (
        <Moon size={18} className="transition-transform duration-300 -rotate-12 hover:rotate-0 text-zinc-900 transform-gpu" />
      )}
    </button>
  );
}
